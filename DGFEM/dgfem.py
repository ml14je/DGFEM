...
class FEM(object):
    ...
    def PlotField2D(self, Nout, xin, yin, uin,
                    title='Field', x_label='$x$', y_label='$y$',
                    name=None, html_output=False, projection3D=False):
        ...
        if not html_output:
            ...
            if projection3D:
                ...
                ax.plot_trisurf(xout.flatten('F'), yout.flatten('F'), TRI,
                                uout.flatten('F'),
                                cmap="seismic")
                pt.show()
            
            else:
                pt.tricontourf(xout.flatten('F'), yout.flatten('F'), TRI,
                               uout.flatten('F'),
                               cmap="seismic")
                pt.show()
    ...
    def FormatData2D(self, Nout, xin, yin, uin):
        ...
        if len(uin.shape) == 1:
            ...
            uout = np.zeros(
                (uin.shape[0], uin.shape[1], interp.shape[0], uin.shape[3])
                )
    ...
    def PlotContour2D(self, Nout, xin, yin, uin, levels,
                      title='Field', x_label='$x$', y_label='$y$',
                      name=None):
        """
        ...
        for n in range(Nlevels):
            ...
            if np.any(ids):
                allx = np.append(
            allx,np.concatenate([xc1[None, ids], xc2[None, ids]]), axis=1
            )
                ally = np.append(
            ally, np.concatenate([yc1[None, ids], yc2[None, ids]]), axis=1
            )

            ids = flag2 & flag3
            if np.any(ids):
                allx = np.append(
            allx, np.concatenate([xc2[None, ids], xc3[None, ids]]), axis=1
            )
                ally = np.append(
            ally, np.concatenate([yc2[None, ids], yc3[None, ids]]), axis=1
            )

            ids = flag1 & flag3
            if np.any(ids):
                allx = np.append(
            allx, np.concatenate([xc1[None, ids], xc3[None, ids]]), axis=1
            )
                ally = np.append(
            ally, np.concatenate([yc1[None, ids], yc3[None, ids]]), axis=1
            )
        ...
        from ppp.Plots import plot_setup
        import matplotlib.pyplot as pt
        fig, ax = plot_setup(x_label, y_label)
        ax.plot(allx, ally, color='r')
        pt.show()

    def PlotContourAdaptive(self, Nout, xin, yin, uin, levels, tolerance,
                      title='Field', x_label='$x$', y_label='$y$',
                      name=None):
        
        raise ValueError("Incomplete")
        # build interpolation matrix (coarse->fine)
        ri, si = np.zeros((len(self.r), 4)), np.zeros((len(self.s), 4))
        ri[:, 0] = 0.5*(-(self.r + self.s)*( 0) + (1 + self.r)*( 0) + (1 + self.s)*(-1) )
        si[:, 0] = 0.5*(-(self.r + self.s)*(-1) + (1 + self.r)*( 0) + (1 + self.s)*( 0) )
        
        ri[:, 1] = 0.5*(-(self.r + self.s)*(-1) + (1 + self.r)*( 0) + (1 + self.s)*(-1) )
        si[:, 1] = 0.5*(-(self.r + self.s)*(-1) + (1 + self.r)*(-1) + (1 + self.s)*( 0) )
        
        ri[:, 2] = 0.5*(-(self.r + self.s)*( 1) + (1 + self.r)*( 0) + (1 + self.s)*( 0) )
        si[:, 2] = 0.5*(-(self.r + self.s)*(-1) + (1 + self.r)*( 0) + (1 + self.s)*(-1) )
        
        ri[:, 3] = 0.5*(-(self.r + self.s)*(-1) + (1 + self.r)*(-1) + (1 + self.s)*( 0) )
        si[:, 3] = 0.5*(-(self.r + self.s)*( 1) + (1 + self.r)*( 0) + (1 + self.s)*( 0) )
        
        interp = self.InterpMatrix2D(ri.flatten('F'), si.flatten('F'))
        Nlevels = len(levels)
        xref = self.x; yref = self.y; uref = uin; Kref = self.K
        
        F = sp((self.Np, self.Np)).tolil()
        sk = 0
        ...
        for nlev in range(Nlevels):
            lev = levels[nlev]
            
            xref, yref, uref = self.x, self.y, uin
            Kref, Jref = self.K, self.J
            
            err = 1
            while err > tolerance:
                
                umin = np.minimum(uref, axis=0)
                umax = np.maximum(uref, axis=0)
                
                refineflag = (umin <= lev) & (umax >= lev)
                toref = np.where(refineflag == True)[0]
                Nref = len(toref)
        ...
        if self.BCType is None:
            ...
            for k in range(self.K): #Element
                for f in range(self.Nfaces): #Face
                    bc = self.BCType[k, f]
                    if bc != 0:
                        ids = k * self.Nfp * self.Nfaces + \
                            f * self.Nfp + np.arange(self.Nfp)
    ...
    def Hrefine2D(self, refineflag):
        ...
        v4 = np.maximum(
            0 + self.Nfaces*np.arange(self.K, dtype=int),
            self.EToF[:, 0] + self.Nfaces * (self.EToE[:, 0] )).astype(int)
        v5 = np.maximum(
            1 + self.Nfaces*np.arange(self.K, dtype=int),
            self.EToF[:, 1] + self.Nfaces * (self.EToE[:, 1] )).astype(int)
        v6 = np.maximum(
            2 + self.Nfaces*np.arange(self.K, dtype=int),
            self.EToF[:, 2] + self.Nfaces * (self.EToE[:, 2] )).astype(int)
        ...
        v4, v5 = (Nv+newids[v4]).astype(int), (Nv+newids[v5]).astype(int)
        v6 = (Nv+newids[v6]).astype(int)
        ...
        self.T = np.concatenate(
            (self.T, np.zeros((3*Nrefine, 3), dtype=int)), axis=0
            )
        self.T[self.K:self.K+3*Nrefine, 0] = \
            np.concatenate((v1, v2, v3)) #first vertices of new elements
        self.T[self.K:self.K+3*Nrefine, 1] = \
            np.concatenate((v4, v5, v6)) # second vertices of new elements
        self.T[self.K:self.K+3*Nrefine, 2] = \
            np.concatenate((v6, v4, v5)) # third vertices of new elements

        # Create boundary condition type for refined elements
        bcsave = np.copy(self.BCType[ref])
        self.BCType[ref] = 0 # no internal faces
        self.BCType =  np.concatenate(
            (self.BCType, np.zeros((3*Nrefine, 3), dtype=int)), axis=0
            )
        ...
        self.VX = np.concatenate(
            (self.VX, np.zeros(np.max(v6) - self.VX.shape[0] + 1)), axis=0
            )
        self.VY = np.concatenate(
            (self.VY, np.zeros(np.max(v6) - self.VY.shape[0] + 1)), axis=0
            )
    ...
    def BuildHNonCon2D(self, NGauss, tol):
        from DGFEM.jacobi import JacobiGQ
        # Build Gauss nodes
...
def Warpfactor(N, rout):
    ...
    from DGFEM.jacobi import JacobiGL, JacobiP
    LGLr = JacobiGL(0, 0, N)
...
def Vandermonde1D(N, r):
    ...
    """
    from DGFEM.jacobi import JacobiP
...
def Vandermonde2D(N, r, s):
    ...
    """
    from DGFEM.jacobi import Simplex2DP
...
def GradVandermonde2D(N, r, s):
    ...
    at (r,s) at order N
    """
    from DGFEM.jacobi import GradSimplex2DP
